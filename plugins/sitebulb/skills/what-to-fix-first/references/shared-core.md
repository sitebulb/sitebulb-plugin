...
- **Both Desktop and Cloud connected** — ask which one to use, as part of the framing interview, before any data pull: "You have both Sitebulb Desktop and Cloud connected — which should I use for this?" Skip the question when the conversation already settles it (the user named the product, or a project already resolved there this session), and reuse the answer for the rest of the session. Never resolve speculatively against both sides — it doubles every discovery call for no gain. Once chosen, bind every capability to that server's tools only.
